...
def generate_app_embedding(
    ...
    """
    logger.info(f"Generating embedding for app: {app.name}...")
    ...
    logger.info(f"Text for app embedding: {text_for_embedding}")
...
def generate_function_embedding(
    ...
    logger.info(f"Text for function embedding: {text_for_embedding}")
    return generate_embedding(
        openai_client, embedding_model, embedding_dimension, text_for_embedding
    )
