...
def _validate_and_fetch_apps_by_name(db: Session, app_names: List[str]) -> List[App]:
    ...
    if missing_names:
        raise ValueError(f"Required apps not found by name / Not Configured: {list(missing_names)}")
...
def list_templates(db: Session, limit: int, offset: int) -> List[AutomationTemplate]:
    """Lists all automation templates with pagination."""
    stmt = select(AutomationTemplate).order_by(AutomationTemplate.name).offset(offset).limit(limit)
    return list(db.execute(stmt).scalars().all())
...
def update_template(
    db: Session, existing_template: AutomationTemplate, template_in: AutomationTemplateUpsert
) -> AutomationTemplate:
    ...
    if "required_app_names" in update_data:
        ...
        existing_template.required_apps = _validate_and_fetch_apps_by_name(db, app_names)
