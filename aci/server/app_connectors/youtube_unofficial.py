...
class YoutubeUnofficial(AppConnectorBase):
    ...
    def get_video_details(
        ...
        """
        logger.info(f"Getting details for video: {video_url}")
    ...
    def get_video_comments(
        ...
        """
        logger.info(f"Getting comments for video: {video_url}")
        ...
        try:
            ...
            # Create downloader and configure proxy
            downloader = YoutubeCommentDownloader()
            
            # Configure proxy on the downloader's session
            if self.proxies:
                downloader.session.proxies.update(self.proxies)
                logger.info(f"Configured proxy for comment downloader: {self.proxies['http']}")
            ...
            raise Exception(f"Failed to get video comments: {e}")
